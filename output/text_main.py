...
data = genfromtxt('dataset/BBC_Sport_termcount_Normal/actual_bbc_sport_count_matrix.txt', skip_header=1, usecols=range(1, 4614)) ##Read the input data
...
for t in range(0, max_gen):
    ...
    for q in range(0, chromosome):
        ...
        A, z, K_copy, lab_copy, sil_score_copy, pbm_index_copy = update_solution(y, data, A, chromosome, sil_score, pbm_index, K, q, max_cluster, features, lab_copy, sil_score_copy, pbm_index_copy, K_copy)
...
file=open('output/actual_bbc_sport_output.txt', 'w')
...
index = final_ARI.index(max(final_ARI))
print "No of clusters detected automatically : " , K[index]
print "Mink score coressponding to maximum ARI : ", final_MS[index]

file.write("\n------------------------------------------------------------------------------------------------------------\n")
file.write("\n------------------------------------------------------------------------------------------------------------\n")
file.write("\n Maximum ari for {0}th chromosome : ".format(index)+ str(final_ARI[index])+'\n')
...
file.write("Final label corresponding to max ARI for {0} chromosome : ".format(index)+ str(lab[index])+'\n')
